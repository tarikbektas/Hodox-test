...
window.th = th
// display targets
if (!window._targetsArea) {
  window._targetsArea = true
  const targetsArea = document.createElement('div')
  document.body.insertBefore(targetsArea, document.body.firstChild)
  let targetsHtml = ''
  Object.keys(targets).forEach((key, i) => {
    targetsHtml += `<button><b>${i}</b> ${key}</button>`
  })
  targetsArea.innerHTML = targetsHtml
}
//
window.hp = hp
const BrainLib = require('/home/he/www/test/brain/dist/brain.js')
const Brain = BrainLib.default
const brain = new Brain
const b = brain

function learn(ipt, targetId) {
  const obj = {target: targetId}
...
function think(ipt) {
  const sortedReasonNames = [
    "currentTree empty",
    "currentTree existed",
    "placeholder existed",
    "placeholder in currentTree",
    "targetNode existed",
    "targetNode is placeholder",
    "targetNode at top",
    "on targetNode middle",
    "targetNode at bottom",
    "placeholder at top",
    "targetNode is the second child of root",
    "targetNode is open",
    "targetNode has children excluding placeholder",
    "targetNode is 1st child",
    "targetNode is last child",
    "at left",
    "at indent right"
  ]
  return b.think((reasonName) => rules[reasonName](ipt), 'target', sortedReasonNames)
}
if (!window.xx) {
  window.xx = b
  window.brain = b
  window.exportJsCode = () => hp.copyTextToClipboard(BrainLib.makeJsCodeByBrain(brain, 'target'))
  window.exportRows = () => hp.copyTextToClipboard(JSON.stringify(b.rows))
  window.showTree = () => {
    brainDebug.tree = BrainLib.getTreeJson(b, 'target')
  }

  // load
  const old = hp.getSessionStorage2().get('brain')
  if (old) {
    b.rows = old
  }
  let oldInfo
  hp.onDOM(document, 'keydown', (e) => {
    if (e.ctrlKey || e.shiftKey) {
      return
    }
    if (e.key === 's') {
      hp.getSessionStorage2().set('brain', b.rows)
      console.log('saved');
      return
    } else if (e.key === 'l') {
      b.rows = hp.getSessionStorage2().get('brain')
      console.log('loaded');
      return
    } else if (e.key === 'x') {
      b.clear()
    ...
    let k = Object.keys(targets)[e.key]
    let tgt = targets[k]
    if (!tgt) return
    window.mp.then(ipt => {
      if (oldInfo === ipt) {
        console.warn('不能同一个info learn多次');
        return
      }
      oldInfo = ipt
      //
  ...
  const elAppendHtml = (el, html) => el.lastChild.insertAdjacentHTML('afterend', html)
  elAppendHtml(document.body, '<div id="brainDebug"></div><style>.table{border-collapse:collapse;}</style>')
  const brainDebug = window.brainDebug = new Vue({
    el: '#brainDebug',
    template: `
<div>
  <node :data="tree"></node>
      ...
      node: {
        ...
        computed: {
          text() {
            ...
            return `${data.error ? '[Error]': ''}${data.reasonValue} - ${data.reasonName} - ${data.targetValue}`
          },
        },
        // watch: {
        //   data: {
        ...
        //         const children2 = []
        //         for (const key of data.children.keys()) {
        //           children2.push({
        //             reasonValue: key,
        //             node: data.children.get(key),
        //           })
        //         }
        //         data.children2 = children2
        //       }
        //     }
        //   }
        // },
        template: `
<div style="padding-left: 2em;" v-if="data">
  <div>{{text}}</div>
  <div v-if="!data.end">
    <node v-for="(t, i) in data.children" :key="i" :data="t"></node>
  </div>
</div>`,
      },
    },
    ...
    methods: {
      p(rule, index) {
        if (this.lj) {
          return rule.currentProbabilities[index].toFixed(4)
        } else {
          return rule.probabilities[index].toFixed(4)
        }
      },
    },
  })
...
export default function autoMoveDragPlaceHolder(draggableHelperInfo) {
  ...
  try {
    if (info.targetNode) {
      console.log(info.targetNode.isDragPlaceHolder ? 'placeholder' : info.targetNode.text);
    } else {
      if (info.currentTree) {
        console.log('no target, in tree');
      } else {
        console.log('no target, out of tree');
      }
    }
  } catch (e) {
    console.warn('log tips 失败', e);
  }
  // 固定学习
  if (!info.dplhEl) {
    // 移动开始时, dplh可能不存在
    // learn(info, 'nothing')
    console.log('no placeholder, do nothing');
  }
  window.mp = new Promise(function(resolve, reject) {
    resolve(info)
  })
  try {
    // console.log('targetNode', info.targetNode && info.targetNode.text);
    // console.log('think start');
    const t = think(info)
    console.log('think result', t);
